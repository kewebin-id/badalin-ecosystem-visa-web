...
export const POST = async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  try {
    ...
    const res = await restApi.post({
      endpoint: endpoints.provider.refund.settle(agencySlug || 'p', id),
      body,
      config: {
        headers: {
          'x-api-key': apiKey,
    ...
    Logger.info(res, { location: 'api/provider/refund/[id]/settle/route.ts - POST' });

    return response.handler(res);
  } catch (error: unknown) {
    Logger.error(error, { location: 'api/provider/refund/[id]/settle/route.ts - POST' });
