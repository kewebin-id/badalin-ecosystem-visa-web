...
export const GET = async (req: NextRequest) => {
  try {
    ...
    if (!apiKey) {
      Logger.error('API_KEY not configured', { location: 'api/visa/dashboard/history/route.ts' });
      return response[500]({ message: 'Internal server error' });
    }

    const restApi = new RestAPI(undefined, session.token as string);
    const res = await restApi.get({
      endpoint: endpoints.visa.dashboard.history,
      config: {
        headers: {
          'x-api-key': apiKey,
    ...
    Logger.info(`GET ${endpoints.visa.dashboard.history} - Response: ${JSON.stringify(res)}`, {
      location: 'api/dashboard/route.ts - GET',
    });

    return response.handler(res);
  } catch (error: unknown) {
    Logger.error(error, { location: 'api/dashboard/route.ts - GET' });
    return response[500]({ message: 'Internal server error' });
  }
};
