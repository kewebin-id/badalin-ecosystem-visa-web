...
interface PilgrimUpdateFormProps {
  id: string;
  initialData: IFamilyMember;
}

export const PilgrimUpdateForm = ({ id, initialData }: PilgrimUpdateFormProps) => {
  ...
  const hasActiveSubmission = initialData?.hasActiveSubmission === true;
  ...
  const ocrMutation = useProcessOcr(
    (data) => {
      ...
      if (data.passportNumber) {
        setValue('passportNumber', data.passportNumber, { shouldDirty: true, shouldValidate: true });
      ...
      if (data.relation) {
        setValue('relation', data?.relation as TRelation, { shouldDirty: true, shouldValidate: true });
        fields.push('relation');
      }
      if (data.gender) {
        setValue('gender', data.gender as 'Male' | 'Female', { shouldDirty: true, shouldValidate: true });
        fields.push('gender');
      }
      ...
      if (data.confidence) setValue('ocrConfidence', data.confidence, { shouldDirty: true, shouldValidate: true });
      if (data.publicUrl) {
    ...
    <div className="space-y-8 relative">
      ...
      <FormProvider {...form}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          {hasActiveSubmission && (
            <div className="bg-warning-50 border border-warning-200 rounded-3xl p-5 flex items-start gap-4 animate-in fade-in duration-500">
              <AlertCircle className="size-5 text-warning-600 shrink-0 mt-0.5" />
              <div className="space-y-1">
                <p className="text-sm font-bold text-warning-800">
                  Data tidak dapat diubah karena sedang dalam proses pengajuan. / Data cannot be changed because it is currently under submission.
                </p>
              </div>
            </div>
          )}
          ...
          <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center gap-4">
            <InputFile
              ...
              onChange={(files) => {
                ...
                setValue('selfieUrl', file?.base64 || '', { shouldValidate: true, shouldDirty: true });
              }}
              capture="user"
              className="w-fit"
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <DocumentUploadField
                ...
                onChange={(files, rawFiles) => {
                  ...
                  setValue('passportUrl', file?.base64 || '', { shouldValidate: true, shouldDirty: true });
                  if (rawFile) {
                ...
                disabled={(ocrMutation.isPending && ocrMutation.variables?.type === 'passport') || hasActiveSubmission}
              />
              <DocumentUploadField
                label={t('ktpPhoto')}
                type="ktp"
                ...
                onChange={(files, rawFiles) => {
                  ...
                  setValue('ktpUrl', file?.base64 || '', { shouldValidate: true, shouldDirty: true });
                  if (rawFile) {
                ...
                isWarningConfirmed={isWarningConfirmed}
                onWarningConfirm={setIsWarningConfirmed}
                disabled={(ocrMutation.isPending && ocrMutation.variables?.type === 'ktp') || hasActiveSubmission}
              />
            </div>
          </section>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <InputFile
                ...
                onChange={(files) => {
                  ...
                  setValue('employmentCertificateUrl', file?.base64 || '', { shouldValidate: true, shouldDirty: true });
                }}
                disabled={hasActiveSubmission}
              />
            </div>
          ...
          <div className="flex justify-end gap-3 rounded-3xl border border-gray-100 shadow-sm p-6 bg-white">
            ...
            <Button
              type="submit"
              disabled={isSubmitting || hasActiveSubmission || !isDirty || !isValid}
